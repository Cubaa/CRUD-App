...
app.get('/users/:id', async (req, res) => {
    ...
    if (foundUser === null)
        return res.send(`Brak użytkownika o id ${id}`);
    res.send(foundUser);
});
...
app.get('/managePanel', (req, res) => {
    res.render('managePanel');
});
app.delete('/users/:id', (req, res) => {
    const { id } = req.params;
    const deleteId = id.substr(1);
    UsersDB.findOneAndDelete({ userId: deleteId + "" }, async function (err, docs) {
        if (err) {
...
app.put('/users/:id', (req, res) => {
    ...
    UsersDB.findOneAndUpdate({ userId: updateId + "" }, { $set: req.body }, { new: true }, (err, doc) => {
        if (err) {
